...
'''
plt.figure()
for i in range(Ti_B_uncs.shape[0]):
    plt.errorbar(dataB.rho, Ti_B_vals[i,:], yerr=Ti_B_uncs[i,:])
plt.xlabel('rho'); plt.ylabel('Ti_B')

plt.figure()
for i in range(omega_tor_B_vals.shape[0]):
    plt.errorbar(dataB.rho, omega_tor_B_vals[i,:], yerr=omega_tor_B_uncs[i,:])
plt.xlabel('rho'); plt.ylabel('omega_tor_B')
'''
